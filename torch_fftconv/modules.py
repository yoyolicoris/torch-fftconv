...
def _check_circular_optimize(
    input_shape: tuple, kernel_shape: tuple, stride: tuple, dilation: tuple
) -> bool:
    for i in range(len(input_shape)):
        if (
            input_shape[i] % stride[i]
            or input_shape[i] % dilation[i]
            or kernel_shape[i] * dilation[i] > input_shape[i]
        ):
            return False
    return (input_shape[-1] // stride[-1]) % 2 == 0 and (
        input_shape[-1] // dilation[-1]
    ) % 2 == 0


def _circular_fft_convnd(
    input: Tensor,
    weight: Tensor,
    ...
    for i in range(2, output.dim()):
        if output.shape[i] < output_size[i - 2]:
            output = torch.cat(
                [output, output[idx + (slice(output_size[i - 2] - output.shape[i]),)]],
                i,
            )
...
class FFTConv1d(nn.Conv1d):
    def forward(self, input: Tensor) -> Tensor:
        if self.padding_mode == "circular" and _check_circular_optimize(
            input.shape[2:], self.weight.shape[2:], self.stride, self.dilation
        ):
            return _circular_fft_convnd(
                input,
                self.weight,
                self.bias,
                self.stride,
