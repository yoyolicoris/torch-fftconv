...
def test_conv1d_circular(
    ...
    x = torch.randn(batch, in_channels, length, requires_grad=True, device=device)
    conv = Conv1d(
        in_channels,
        out_channels,
        kernel_size,
...
@pytest.mark.parametrize("stride", [1, 3])
@pytest.mark.parametrize("dilation", [1, 2, 3])
@pytest.mark.parametrize("padding", [0, 6])
@pytest.mark.parametrize("bias", [True, False])
def test_conv3d_circular(
    batch,
    length,
    in_channels,
    out_channels,
...
@pytest.mark.parametrize("length", [1021])
